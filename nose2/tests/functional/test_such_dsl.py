...
class TestSuchDSL(FunctionalTestCase):
    def test_runs_example(self):
        proc = self.runIn("such", "-v", "--plugin=nose2.plugins.layers", "test_such")
    ...
    def test_param_plugin_with_such(self):
        ...
        self.assertTestRunOutputMatches(
            proc, stderr="test 0001: should do bar and extra:3"
        )

    def test_such_without_layers_plugin(self):
        proc = self.runIn("such", "-v", "test_such_without_layers")
        self.assertTestRunOutputMatches(proc, stderr=r"FAILED")
        self.assertTestRunOutputMatches(
            proc, stderr=such.LAYERS_PLUGIN_NOT_LOADED_MESSAGE
        )

    def test_testsetup_on_higher_layer_with_test(self):
        proc = self.runIn(
            "scenario/layers_setups",
            "--plugin=nose2.plugins.layers",
            "-v",
    ...
    def test_long_setup(self):
        proc = self.runIn(
            "such", "-v", "--plugin=nose2.plugins.layers", "test_such_timing"
        )
